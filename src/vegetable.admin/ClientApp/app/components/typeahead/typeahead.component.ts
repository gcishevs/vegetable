...
export class NgbdTypeaheadBasic {
    ...
    constructor(http: Http) {
        http.get('/api/AddressData/AddressItemList?type=0').subscribe(result => {
            this.dataItems = result.json();
        });
    }
    ...
    search = (text$: Observable<string>) =>
        text$
            ...
            .map(term => term.length < 2 ? []
                : this.dataItems.filter(v => new RegExp(term, 'gi').test(v)).splice(0, 10));
}
